...
package net.arbaro.tree;

import java.util.Enumeration;
import net.arbaro.params.*;
import net.arbaro.transformation.*;
...
public class Segment{
	...
	public Segment(/* Params params, LevelParams lparams, */
			Stem stm, int inx, Transformation trf, double r1, double r2){
		index = inx;
		transf = trf;
		rad1 = r1;
		rad2 = r2;
	...
	private void makeSubsegments(int cnt){
		...
		for(int i = 0; i < cnt + 1; i++){
			...
			// System.err.println("SUBSEG: pos: "+ pos+" rad: "+rad+" inx:
			// "+index+" len: "+length);
	 ...
	 * 
	 * @param cnt
	 *            the number of subsegments, should be higher
	...
	private void makeHelix(int cnt){
		...
		for(int i = 0; i < cnt + 1; i++){
			Vector pos = new Vector(rad * Math.cos(2 * Math.PI * i / cnt) - rad, rad * Math.sin(2 * Math.PI * i / cnt), i * length / cnt);
	 ...
	 *
	 * @param trf
	 *            the transformation of the substem
	 * @param where
	 ...
	 * @param meshpart
	 *            the mesh part where the points should be added
	 * @param donttrf
	 *            if true, the transformation is not applied to the section
	 *            points
	 */
	/*
	 * // TODO should be obsolete when Traversals are working
	 * private void createSectionMeshpoints(Vector pos, double rad, MeshPart
	 * meshpart,
	 ...
	 * // for Lobes ensure that points are near lobes extrema, but not exactly
	 * there
	 ...
	 * Vector pt = new
	 * Vector(Math.cos(angle*Math.PI/180),Math.sin(angle*Math.PI/180),0);
	 ...
	 * subsegments.size());
	 * pt =
	 * pt.mul(rad1*(1.0+par.LobeDepth*Math.cos(par.Lobes*angle*Math.PI/180.0)));
	 * } else {
	 ...
	 * 
	 * @param meshpart
	 *            the mesh part, to wich the segment should be added
	 */
	 ...
	 * 
	 * double vLength =
	 * stem.getLength()+stem.stemRadius(0)+stem.stemRadius(stem.length);
	 ...
	 * ((MeshSection)meshpart.firstElement()).setNormalsToVector(transf.getZ().
	 * mul(-1));
	 * 
	...
	public boolean traverseStem(StemTraversal traversal) throws TraversalException{
		if(traversal.enterSegment(this)) // enter this tree?
		{
			...
			while(s.hasMoreElements())
				if(!((Subsegment)s.nextElement()).traverseStem(traversal)) break;
		}
