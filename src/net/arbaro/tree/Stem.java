...
import net.arbaro.params.LevelParams;
import net.arbaro.params.Params;
...
public class Stem {
	...
	*/
	 ...
	 */
	@SuppressWarnings("unchecked") Stem clone(Transformation trf, int start_segm) {
...
//		tree.minMaxTest(minPoint);
	...
	@SuppressWarnings("unchecked") int makeSegments(int start_seg,int end_seg) {
	...
	void prepareSubstemParams() {
		...
		if (stemlevel==0) {
			...
			substemsPerSegment = substem_cnt / segmentCount / (1-par.BaseSize);
			DBG("Stem.prepare_substem_params(): stems_max: "+ substem_cnt 
					+ " substems_per_segment: " + substemsPerSegment);
		} else if (par.preview) {
			substem_cnt = stems_max;
			substemsPerSegment = substem_cnt / segmentCount;
		} else if (stemlevel==1) {
	 ...
	 * @param segment
	 */
	@SuppressWarnings("unchecked") void makeSubstems(Segment segment) {
	 ...
	 * @param segment
	 */
	@SuppressWarnings("unchecked") void makeLeaves(Segment segment) {
	 ...
	 */
	@SuppressWarnings("unchecked") int makeClones(Transformation trf,int nseg) {
		// splitting
