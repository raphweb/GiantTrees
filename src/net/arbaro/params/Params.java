...
public class Params{
	...
	public void toXML(PrintWriter w) throws ParamError{
		...
		for(int i = 0; i <= Math.min(Levels, 3); i++){
			levelParams[i].toXML(w, i == Levels); // i==Levels => leaf level
													// only
		}
		w.println("  </species>");
		w.println("</arbaro>");
	...
	void fromDB() throws ParamError{
		...
		Leaves = Leaves != -1 ? getIntParam("Leaves") : 0; // Don't render
															// leaves if
															// explicitly
															// turning them off
	...
	public void prepare() throws ParamError{
		...
		if(Lobes > 0) {
			...
			levelParams[0].mesh_points = Math.max(levelParams[0].mesh_points, (int)(4 * (1 + 2 * mesh_quality)));
		}
		for(int i = 1; i < 4; i++){
			levelParams[i].mesh_points = Math.max(3, (int)(levelParams[i].mesh_points * (1 + 1.5 * mesh_quality)));
		}
	...
	public double getShapeRatio(double ratio, int shape){
		...
		switch(shape){
			...
			case ENVELOPE:
				...
				// tested in prepare() default: throw new ErrorParam("Shape must
				// be between 0 and 8");
		}
	...
	@SuppressWarnings("unchecked") public TreeMap getParamGroup(int level, String group){
	...
	@SuppressWarnings("unchecked") private void intParam(String name, int min, int max, int deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new IntParam(name, min, max, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	@SuppressWarnings("unchecked") private void shapeParam(String name, int min, int max, int deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new ShapeParam(name, min, max, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	@SuppressWarnings("unchecked") private void int4Param(String name, int min, int max, int deflt0, int deflt1, int deflt2, int deflt3, String group, String short_desc, String long_desc){
		int[] deflt = { deflt0, deflt1, deflt2, deflt3 };
		order++;
		for(int i = 0; i < 4; i++){
			name = "" + i + name.substring(1);
			paramDB.put(name, new IntParam(name, min, max, deflt[i], group, i, order, short_desc, long_desc));
		}
	}

	@SuppressWarnings("unchecked") private void dblParam(String name, double min, double max, double deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new FloatParam(name, min, max, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	@SuppressWarnings("unchecked") private void dbl4Param(String name, double min, double max, double deflt0, double deflt1, double deflt2, double deflt3, String group, String short_desc,
			String long_desc){
		double[] deflt = { deflt0, deflt1, deflt2, deflt3 };
		order++;
		for(int i = 0; i < 4; i++){
			name = "" + i + name.substring(1);
			paramDB.put(name, new FloatParam(name, min, max, deflt[i], group, i, order, short_desc, long_desc));
		}
	}

	@SuppressWarnings("unchecked") private void lshParam(String name, String deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new LeafShapeParam(name, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	@SuppressWarnings("unchecked") private void woodTypeParam(String name, String deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new WoodTypeParam(name, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	@SuppressWarnings("unchecked") private void strParam(String name, String deflt, String group, String short_desc, String long_desc){
		paramDB.put(name, new StringParam(name, deflt, group, AbstractParam.GENERAL, order++, short_desc, long_desc));
	}

	private void registerParams(){
		order = 1;

		strParam("Species", "default", "SHAPE", "the tree's species",
		...
		intParam("0BaseSplits", 0, Integer.MAX_VALUE, 0, "SHAPE", "stem splits at base of trunk",
		...
		// "<strong>ZScale</strong> and ZScaleV are not described in the
		// Weber/Penn paper.<br>\n"+
		...
		// "ZScale and <strong>ZScaleV</strong> are not described in the
		// Weber/Penn paper.<br>\n"+
		// "so theire meaning is unclear and they aren't used at the moment\n"
		// );

		dblParam("Ratio", 0.000001, Double.POSITIVE_INFINITY, 0.05, "TRUNK", "trunk radius/length ratio",
		...
		dblParam("RatioPower", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 1.0, "SHAPE", "radius reduction",
		...
		dblParam("Flare", -1.0, Double.POSITIVE_INFINITY, 0.5, "TRUNK", "exponential expansion at base of tree",
		...
		intParam("Lobes", 0, Integer.MAX_VALUE, 0, "UNUSED", "sinusoidal cross-section variation",
		...
		dblParam("LobeDepth", 0, Double.POSITIVE_INFINITY, 0, "UNUSED", "amplitude of cross-section variation",
		...
		intParam("Leaves", Integer.MIN_VALUE, Integer.MAX_VALUE, 0, "LEAVES", "number of leaves per stem",
		...
		dblParam("LeafScale", 0.000001, Double.POSITIVE_INFINITY, 8, "LEAVES", "leaf length/width",
				"<strong>LeafScale</strong> is the length of the leaf in meters.<br>\n" + "The unit leaf is scaled in x/z-direction \n");

		dblParam("LeafScaleX", 0.000001, Double.POSITIVE_INFINITY, 0.75, "LEAVES", "fractional leaf height",
		...
		dblParam("LeafStemLen", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0, "LEAVES", "fractional leaf stem length",
		...
		dblParam("AttractionUp", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0.0, "SHAPE", "upward/downward growth tendency",
		...
		dblParam("PruneRatio", 0.0, 1.0, 0.0, "PRUNING", "fractional effect of pruning",
		...
		dblParam("PrunePowerLow", 0.0, Double.POSITIVE_INFINITY, 0.5, "PRUNING", "curvature of envelope",
		...
		dblParam("PrunePowerHigh", 0.0, Double.POSITIVE_INFINITY, 0.5, "PRUNING", "curvature of envelope",
				"<strong>PrunePowerHigh</strong> describes the envelope curve above the peak.<br>\n"
						+ "A value of 1 means linear decreasing. Higher values means concave,<br>\n" + "lower values convex curve.<br>\n"
						+ "This parameter is used for the shape \"envelope\" too, even if PruneRatio is off.\n");

		dblParam("0Scale", 0.000001, Double.POSITIVE_INFINITY, 1.0, "TRUNK", "extra trunk scaling",
		...
		dblParam("0ScaleV", 0.0, Double.POSITIVE_INFINITY, 0.0, "TRUNK", "variation for extra trunk scaling",
		...
		dbl4Param("nLength", 0.0000001, Double.POSITIVE_INFINITY, 1.0, 0.5, 0.5, 0.5, "LENTAPER", "fractional trunk scaling",
		...
		dbl4Param("nLengthV", 0.0, Double.POSITIVE_INFINITY, 0.0, 0.0, 0.0, 0.0, "LENTAPER", "variation of fractional trunk scaling",
				"<strong>nLengthV</strong> is the variation of the length given by nLength.<br>\n");

		dbl4Param("nTaper", 0.0, 2.99999999, 1.0, 1.0, 1.0, 1.0, "LENTAPER", "cross-section scaling",
		...
		dbl4Param("nSegSplits", 0, Double.POSITIVE_INFINITY, 0, 0, 0, 0, "SPLITTING", "stem splits per segment",
		...
		dbl4Param("nSplitAngle", 0, 180, 0, 0, 0, 0, "SPLITTING", "splitting angle",
		...
		dbl4Param("nSplitAngleV", 0, 180, 0, 0, 0, 0, "SPLITTING", "splitting angle variation",
				"<strong>nSplitAngleV</strong> is the variation of the splitting angle. See nSplitAngle.<br>\n");

		int4Param("nCurveRes", 1, Integer.MAX_VALUE, 3, 3, 1, 1, "CURVATURE", "curvature resolution",
		...
		dbl4Param("nCurve", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0, 0, 0, 0, "CURVATURE", "curving angle",
		...
		dbl4Param("nCurveV", -90, Double.POSITIVE_INFINITY, 0, 0, 0, 0, "CURVATURE", "curving angle variation",
		...
		dbl4Param("nCurveBack", Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY, 0, 0, 0, 0, "CURVATURE", "curving angle upper stem half",
		...
		dbl4Param("nDownAngle", -179.9999999, 179.999999, 0, 30, 30, 30, "BRANCHING", "angle from parent",
				"<strong>nDownAngle</strong> is the angle between a stem and it's parent.<br>\n");

		dbl4Param("nDownAngleV", -179.9999999, 179.9999999, 0, 0, 0, 0, "BRANCHING", "down angle variation",
		...
		dbl4Param("nRotate", -360, 360, 0, 120, 120, 120, "BRANCHING", "spiraling angle",
		...
		int4Param("nBranches", 0, Integer.MAX_VALUE, 1, 10, 5, 5, "BRANCHING", "number of branches",
		...
		dbl4Param("nBranchDist", 0, 1, 0, 1, 1, 1, "BRANCHING", "branch distribution along the segment",
		...
		// "<strong>OutFormat</strong> defines the format of the outputfile for
		// rendering.<br>\n");
		//
		// intParam("RenderWidth",15,6000,600,
		// "RENDER","the width of the rendered image",
		// "<strong>RenderWidth</strong> is the width of the rendered
		// image,<br>\n"+
		// "if you render a scene with the tree from Arbaro.");
		//
		// intParam("RenderHeight",20,8000,800,
		// "RENDER","the height of the rendered image",
		// "<strong>RenderHeight</strong> is the height of the rendered
		// image,<br>\n"+
		// "if you render a scene with the tree from Arbaro.");
		//
		// intParam("Seed",0,Integer.MAX_VALUE,13,
		// "RENDER","the random seed",
		// "<strong>Seed</strong> is the seed for initializing the random
		// generator<br>\n"+
		// "making the tree individual. So you can think of it as the tree's
		// seed too.");
	...
	public void enableDisable(){
		...
		enable = (((IntParam)getParam("Leaves")).intValue() != 0 && ((IntParam)getParam("Levels")).intValue() > 1);
		...
		enable = (((IntParam)getParam("Shape")).intValue() == 8 || ((FloatParam)getParam("PruneRatio")).doubleValue() > 0);
		...
		for(int i = 0; i < 4; i++){
			...
			// used for leaves
			enable = enable || (((IntParam)getParam("Leaves")).intValue() != 0 && i == ((IntParam)getParam("Levels")).intValue());
		...
		for(int i = 0; i < ((IntParam)getParam("Levels")).intValue() && i < 4; i++){
			...
			enable = (((FloatParam)getParam("" + i + "SegSplits")).doubleValue() > 0) || (i == 0 && ((IntParam)getParam("0BaseSplits")).intValue() > 0);
			getParam("" + i + "SplitAngle").setEnabled(enable);
			getParam("" + i + "SplitAngleV").setEnabled(enable);
