...
import net.arbaro.params.IntParam;
...
public final class Workplace{
	...
	void createGUI(){
			...
			@Override public void stateChanged(ChangeEvent e){
				...
				try{
					...
					if(level == AbstractParam.GENERAL) {
						if(group.equals("LEAVES") || group.equals("LEAVESADD")) previewTree.setShowLevel(((IntParam)tree.getParam("Levels")).intValue());
						else previewTree.setShowLevel(1);
					}
					else{
		...
		imageLabel = new JLabel("", shapeIcon, SwingConstants.CENTER);
		imageLabel.setBorder(
				BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(2, 2, 2, 2), "Tree shape", TitledBorder.CENTER, TitledBorder.BOTTOM));
	...
	private JPanel createFrontView(){
		...
		rotator.setBorder(BorderFactory.createMatteBorder(0, 0, 0, 1, frontViewWithSlider.getBackground()));
	...
	class FileNewAction extends AbstractAction{
		...
		@Override public void actionPerformed(ActionEvent e){
			...
			try{
				...
				setModified(false);
				JOptionPane.showMessageDialog(frame, err.getMessage(), "Parameter Error", JOptionPane.ERROR_MESSAGE);
			}
			catch(Exception err){
				System.err.println(err);
				err.printStackTrace();
	...
	public void openFile(File file){
		...
		try{
			...
			int input = JOptionPane.showConfirmDialog(frame, file.getName() + " does not exist. Do you want to create it?", "File not found",
	...
	boolean shouldSave(){
		if(modified) {
			int result = JOptionPane.showConfirmDialog(frame, "Some parameters are modified. Should the tree definition be saved?", "Tree definition modified",
		...
		else return true; // not modified, can proceed
	}
	...
	boolean fileSave(){
		try{
			...
			JOptionPane.showMessageDialog(frame, err.getMessage(), "File not found", JOptionPane.ERROR_MESSAGE);
		}
		catch(IOException err){
			JOptionPane.showMessageDialog(frame, err.getMessage(), "Output error", JOptionPane.ERROR_MESSAGE);
		}
		return false;
	}
	...
	class HelpAboutAction extends AbstractAction{
		...
		@Override public void actionPerformed(ActionEvent e){
			JOptionPane.showMessageDialog(frame, net.arbaro.arbaro.programName, "About Arbaro", JOptionPane.INFORMATION_MESSAGE, aboutIcon);
		}
	}
}
...
// setToolTipText("tree species name, used for object names in the POVRay
// file");
//
