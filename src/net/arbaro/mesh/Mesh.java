...
public class Mesh extends java.util.Vector{
			...
			if(parts.hasMoreElements()) {
				...
				if(level >= 0) {
					while(part.stem.stemlevel != level){
						if(parts.hasMoreElements()) part = (MeshPart)parts.nextElement();
						else{
							part = null;
							return;
						}
	 ...
	 * @param meshpart
	 */
	@SuppressWarnings("unchecked") public void addMeshpart(MeshPart meshpart){
		addElement(meshpart);
		if(firstMeshPart[meshpart.stem.stemlevel] < 0) firstMeshPart[meshpart.stem.stemlevel] = size() - 1;
	}

	public Enumeration allVertices(boolean UVVertices){
		if(UVVertices) return new UVVertexEnumerator();
		else return new VertexEnumerator();
	}
