...
import net.arbaro.tree.DefaultTreeTraversal;
import net.arbaro.tree.Leaf;
...
class OBJLeafExporter extends DefaultTreeTraversal{
	 ...
	 * 
	 */
	public OBJLeafExporter(PrintWriter pw, LeafMesh leafMesh, int leafVertexOffset){
		super();
		this.w = pw;
...
class OBJLeafFaceExporter extends OBJLeafExporter{
	 ...
	 */
	public OBJLeafFaceExporter(PrintWriter pw, LeafMesh leafMesh, int leafVertexOffset, int uvVertexOffset, long smoothingGroup, boolean outputLeafUVs,
...
class OBJLeafVertexExporter extends OBJLeafExporter{
	 ...
	 * @param leafVertexOffset
	 */
	public OBJLeafVertexExporter(PrintWriter pw, LeafMesh leafMesh, int leafVertexOffset, String type){
		super(pw, leafMesh, leafVertexOffset);
		this.type = type;
	}
...
public final class OBJExporter extends Exporter{
	...
	@Override public void write() throws ExportError{
		...
		try{
			...
			writeStemFaces();
			// writeLeafFaces();
			OBJLeafFaceExporter faceExporter = new OBJLeafFaceExporter(w, leafMesh, vertexOffset, uvVertexOffset, smoothingGroup, outputLeafUVs, outputStemUVs);
	...
	private void writeLeafVertices(String type) throws Exception{
		...
		if(type == "vt") {
			...
			OBJLeafVertexExporter vertexExporter = new OBJLeafVertexExporter(w, leafMesh, vertexOffset, type);
	...
	private void writeStemFaces() throws Exception{
		...
		for(int stemLevel = 0; stemLevel < tree.params.Levels; stemLevel++){
			...
			for(Enumeration parts = mesh.allParts(stemLevel); parts.hasMoreElements();){
	...
	private void writeUVVertex(UVVector v){
		w.println("vt " + frm.format(v.u) + " " + frm.format(v.v) + " " + frm.format(0));
	}
