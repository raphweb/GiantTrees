...
package net.arbaro.export;

import java.io.PrintWriter;
import java.util.Enumeration;
import net.arbaro.mesh.*;
import net.arbaro.params.FloatFormat;
import net.arbaro.transformation.Vector;
import net.arbaro.tree.*;
...
class POVMeshLeafExporter extends DefaultTreeTraversal{
	 ...
	 * 
	 */
	public POVMeshLeafExporter(PrintWriter pw, LeafMesh leafMesh, long leafVertexOffset){
		super();
		this.w = pw;
	...
	@Override public boolean enterTree(Tree tree){
	...
	void writeVector(Vector v){
		w.print("<" + fmt.format(v.getX()) + "," + fmt.format(v.getZ()) + "," + fmt.format(v.getY()) + ">");
	}
...
class POVMeshLeafFaceExporter extends POVMeshLeafExporter{
	...
	public POVMeshLeafFaceExporter(PrintWriter pw, LeafMesh leafMesh, long leafVertexOffset){
		super(pw, leafMesh, leafVertexOffset);
	}

	@Override public boolean visitLeaf(Leaf l){
		String indent = "    ";

		for(int i = 0; i < leafMesh.getShapeFaceCount(); i++){
			Face face = leafMesh.shapeFaceAt(i);
			w.print("<" + (leafVertexOffset + face.points[0]) + "," + (leafVertexOffset + face.points[1]) + "," + (leafVertexOffset + face.points[2]) + ">");
...
class POVMeshLeafNormalExporter extends POVMeshLeafExporter{
	 ...
	 */
	public POVMeshLeafNormalExporter(PrintWriter pw, LeafMesh leafMesh, long leafVertexOffset){
		super(pw, leafMesh, leafVertexOffset);
		// TODO Auto-generated constructor stub
	}

	@Override public boolean visitLeaf(Leaf l) throws TraversalException{
		String indent = "    ";
...
class POVMeshLeafUVFaceExporter extends POVMeshLeafExporter{
	 ...
	 * @param leafVertexOffset
	 */
	public POVMeshLeafUVFaceExporter(PrintWriter pw, LeafMesh leafMesh, long leafVertexOffset){
		super(pw, leafMesh, leafVertexOffset);
		// TODO Auto-generated constructor stub
	}
	...
	@Override public boolean visitLeaf(Leaf l){
		...
		for(int i = 0; i < leafMesh.getShapeFaceCount(); i++){
			Face face = leafMesh.shapeFaceAt(i);
			w.print("<" + (/* leafFaceOffset+ */face.points[0]) + "," + (/* leafFaceOffset+ */face.points[1]) + "," + (/* leafFaceOffset+ */face.points[2])
					+ ">");
			if(i < leafMesh.getShapeFaceCount() - 1) {
				w.print(",");
			}
...
class POVMeshLeafVertexExporter extends POVMeshLeafExporter{
	 ...
	 * @param leafVertexOffset
	 */
	public POVMeshLeafVertexExporter(PrintWriter pw, LeafMesh leafMesh, long leafVertexOffset){
		super(pw, leafMesh, leafVertexOffset);
		// TODO Auto-generated constructor stub
	}
...
public class POVMeshExporter extends Exporter{
	...
	@Override public void write() throws ExportError{
		try{
			...
			w.println();
			w.println(net.arbaro.arbaro.programName);
			w.println();
			tree.params.toXML(w);
			w.println("************************************************/");
	...
	private void writeLeaves() throws Exception{
		// double leafLength =
		// tree.params.LeafScale/Math.sqrt(tree.params.LeafQuality);
		// double leafWidth =
		// tree.params.LeafScale*tree.params.LeafScaleX/Math.sqrt(tree.params.LeafQuality);
		// LeafMesh mesh = new
		// LeafMesh(tree.params.LeafShape,leafLength,leafWidth,tree.params.LeafStemLen);
		...
		if(leafCount > 0) {
			...
			if(outputLeafNormals) {
				// w.println(" normal_vectors {
				// "+mesh.getShapeVertexCount()*leafCount);
	 ...
	 * incLeavesProgressCount();
	 * throw new Exception(
	 * "Not implemented: if using normals for leaves use factor "+
	...
	private void writeStems() throws Exception{
		...
		if(outputStemUVs) elements += face_cnt; // passes=passes++; // for the
												// faces
		progress.beginPhase("Writing stem mesh",
				elements/* mesh.size()*passes */);
	...
	private void writeStemPoints(String indent){
		...
		int i = 0;
		for(Enumeration vertices = mesh.allVertices(false); vertices.hasMoreElements();){
	...
	@SuppressWarnings("rawtypes")
	public void writeStemFaces(boolean uv, String indent) throws MeshError{

		int j = 0;
		for(Enumeration faces = mesh.allFaces(0, uv, -1 /* all levels */); faces.hasMoreElements();){

			Face face = (Face)faces.nextElement();
			w.print("<" + face.points[0] + "," + face.points[1] + "," + face.points[2] + ">");
	...
	@SuppressWarnings("rawtypes")
	private void writeStemNormals(String indent) throws MeshError{

		int i = 0;

		try{
			for(Enumeration parts = mesh.elements(); parts.hasMoreElements();){
	...
	private void writeStemUVs(/* MeshPart mp, */ String indent){
		...
		int j = 0;
		for(Enumeration vertices = mesh.allVertices(true); vertices.hasMoreElements();){
