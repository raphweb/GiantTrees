...
public class Draw3d{
	public class Point2D{
		public int p, q;
		public Point2D(final int p, final int q){this.p = p; this.q = q;}
	}
	public enum RenderOrientation{INVERTED, NORMAL}
	...
	public Draw3d(final Location refPoint, final double noiseIntensity, final TreeType treeType,
			final WorldChangeTracker changeTracker, final RenderOrientation renderOrientation){
	...
	public void drawCone(final Vector l1, final double rad1, final Vector l2, final double rad2, final int level){
	...
	public Vector toMcVector(final net.arbaro.transformation.Vector arbVec){
	...
	private double calculateNoiseOffset(final int x, final int y, final int z, final int multiplier, final int level){
		final double levelScale = ((double)(4 - level) / (double)4);
		return (noise.noise(x * .25, y * .25, z * .25) + 1) * noiseIntensity * multiplier * levelScale;
	}

	private List<Point2D> plotCircle(final int cx, final int cy, final int z, final int r, final int level){
		final List<Point2D> points2d = new LinkedList<>();

		final int rSquare = r * r;
		for(int x = -r - 8; x <= (r + 8); x++){
			for(int y = -r - 8; y <= (r + 8); y++){
				final double noiseOffset = calculateNoiseOffset((x + cx), (y + cy), (z), 4, level);
		...
		for(int x = -rCeil; x <= rCeil; x++){
			for(int y = -rCeil; y <= 0; y++){
				for(int z = -rCeil; z <= rCeil; z++){
					...
					final double noiseOffset = calculateNoiseOffset((x + pos.getBlockX()), (y + pos.getBlockY()), (z + pos.getBlockZ()), 2, 0);
	...
	private List<Vector> plotEllipsoid(final Vector pos, final double a, final double b, final double c, final int level){
		final List<Vector> points = new LinkedList<>();
		...
		for(int x = -halfSize; x <= halfSize; x++){
			for(int y = -halfSize; y <= halfSize; y++){
				for(int z = -halfSize; z <= halfSize; z++){
					final double left = ((x * x) / (a * a)) + ((y * y) / (b * b)) + ((z * z) / (c * c));
					final double noiseOffset = calculateNoiseOffset((x + pos.getBlockX()), (y + pos.getBlockY()), (z + pos.getBlockZ()), 1, level);
	...
	private List<Point2D> plotLine2d(final int x1, final int y1, final int x2, final int y2){
		final List<Point2D> points2d = new LinkedList<>();
	...
	private List<Vector> plotLine3d(final Vector l1, final Vector l2, final Orientation orientation){
		final List<Vector> locations = new LinkedList<>();
		if(orientation == Orientation.xMajor) /* x dominant */
		{
			final List<Point2D> xy = plotLine2d(l1.getBlockX(), l1.getBlockY(), l2.getBlockX(), l2.getBlockZ());
			final List<Point2D> xz = plotLine2d(l1.getBlockX(), l1.getBlockZ(), l2.getBlockX(), l2.getBlockZ());
		...
		{
			final List<Point2D> yx = plotLine2d(l1.getBlockY(), l1.getBlockX(), l2.getBlockY(), l2.getBlockX());
			final List<Point2D> yz = plotLine2d(l1.getBlockY(), l1.getBlockZ(), l2.getBlockY(), l2.getBlockZ());
		...
		{
			final List<Point2D> zx = plotLine2d(l1.getBlockZ(), l1.getBlockX(), l2.getBlockZ(), l2.getBlockX());
			final List<Point2D> zy = plotLine2d(l1.getBlockZ(), l1.getBlockY(), l2.getBlockZ(), l2.getBlockY());
	...
	private List<Vector> plotSphere(final Vector pos, final double r, final int level){
		final List<Vector> points = new LinkedList<>();
		final int rCeil = (int)Math.ceil(r) + 4;
		final double r2 = r * r;
		for(int x = -rCeil; x <= rCeil; x++){
			for(int y = -rCeil; y <= rCeil; y++){
				for(int z = -rCeil; z <= rCeil; z++){
					final double left = (x * x) + (y * y) + (z * z);
					final double noiseOffset = calculateNoiseOffset((x + pos.getBlockX()), (y + pos.getBlockY()), (z + pos.getBlockZ()), 2, level);
					if(left <= (r2 + (noiseOffset * noiseOffset))) {
						points.add(new Vector(x, y, z).add(pos));
					}
				}
